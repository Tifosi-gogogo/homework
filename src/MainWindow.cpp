...
QString saveFilePath() {
    ...
    return dir.filePath(QStringLiteral("save.json"));
}

QSet<QString> validOutfitIds() {
    QSet<QString> ids;
    for (const OutfitItem& item : outfitCatalog()) {
        ids.insert(item.id);
    }
    return ids;
}

QSet<QString> validRacketIds() {
    QSet<QString> ids;
    for (const RacketItem& item : racketCatalog()) {
        ids.insert(item.id);
    }
    return ids;
}
...
QJsonArray stringSetToJsonArray(const QSet<QString>& values) {
    std::vector<QString> sorted(values.begin(), values.end());
...
QSet<QString> readOwnedIds(const QJsonValue& value, const QSet<QString>& validIds, const QSet<QString>& defaults) {
    QSet<QString> result = defaults;
    if (!value.isArray()) {
        return result;
    }
...
QJsonObject storeToJson(const PlayerStore& store) {
    QJsonObject object;
    object.insert(QStringLiteral("coins"), store.coins);
    object.insert(QStringLiteral("ownedOutfits"), stringSetToJsonArray(store.ownedOutfits));
    object.insert(QStringLiteral("ownedRackets"), stringSetToJsonArray(store.ownedRackets));
    return object;
}

void readStoreFromJson(const QJsonObject& object, PlayerStore& store,
                       const QSet<QString>& outfitIds, const QSet<QString>& racketIds) {
    const QJsonValue coinsValue = object.value(QStringLiteral("coins"));
    if (coinsValue.isDouble()) {
        store.coins = std::max(0, coinsValue.toInt(store.coins));
    }

    store.ownedOutfits = readOwnedIds(object.value(QStringLiteral("ownedOutfits")),
                                      outfitIds, store.ownedOutfits);
    store.ownedRackets = readOwnedIds(object.value(QStringLiteral("ownedRackets")),
                                      racketIds, store.ownedRackets);
}
...
bool MainWindow::loadProgress() {
    ...
    if (parseError.error != QJsonParseError::NoError || !document.isObject()) {
        qWarning() << "Failed to read save file:" << parseError.errorString();
        return false;
    }

    const QJsonObject root = document.object();
    const QSet<QString> outfitIds = validOutfitIds();
    const QSet<QString> racketIds = validRacketIds();
    readStoreFromJson(root.value(QStringLiteral("player1")).toObject(), p1Store_, outfitIds, racketIds);
    readStoreFromJson(root.value(QStringLiteral("player2")).toObject(), p2Store_, outfitIds, racketIds);
    return true;
}

void MainWindow::saveProgress() const {
    QJsonObject root;
    root.insert(QStringLiteral("version"), kSaveVersion);
    root.insert(QStringLiteral("player1"), storeToJson(p1Store_));
    root.insert(QStringLiteral("player2"), storeToJson(p2Store_));

    QSaveFile file(saveFilePath());
    if (!file.open(QIODevice::WriteOnly)) {
        qWarning() << "Failed to open save file for writing:" << file.errorString();
        return;
    }

    const QByteArray data = QJsonDocument(root).toJson(QJsonDocument::Indented);
    if (file.write(data) != data.size()) {
        qWarning() << "Failed to write save file:" << file.errorString();
        file.cancelWriting();
        return;
    }
    if (!file.commit()) {
        qWarning() << "Failed to commit save file:" << file.errorString();
    }
}
